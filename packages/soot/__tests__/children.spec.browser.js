...
describe("Children - (non-JSX)", () => {
        ...
        return (
          <div className="tab-group">
            {[
              ...
              <Tab onSelect={create} id="add" key="add" title="Add" />
            ]}
          </div>
        );
      }
  ...
  describe("nonKeyed - children", function() {
    it("Should push to correct location when it keyed list has siblings #1", function() {
      const _tabs = [{ title: "Item A" }, { title: "Item B" }];
    ...
    it("Inline text element before array list #3", function() {
      const _tabs = [];
    ...
    it("Inline text element after array list #2", function() {
      const _tabs = [];
  ...
  describe("mixed children edge cases", function() {
    it("NONKEYED - should remove children from correct location when there is dynamic static item", function() {
      ...
      function Looper({ collectionOne, visibleStatic }) {
        const content = [visibleStatic ? <Loop i={-1} text="static" /> : null];

        for (let i = 0; i < collectionOne.length; i++) {
          content.push(<Loop text={collectionOne[i]} />);
        }
    ...
    it("NONKEYED - should remove children from correct location when there is 2 dynamic static items and 2 lists", function() {
      ...
      function Looper({
        ...
        const content = [visibleStaticOne ? <Loop text="static" /> : null];
        content.push(...collectionOne.map(text => <Loop text={text} />));
        content.push(visibleStaticTwo ? <Loop text="static" /> : null);
        content.push(...collectionTwo.map(text => <Loop text={text} />));
        return <div className="c">{content}</div>;
      }
    ...
    it("KEYED - should remove children from correct location when there is dynamic static item", function() {
      ...
      function Looper({ collectionOne, visibleStatic }) {
        const content = [
          visibleStatic ? <Loop key={-1} i={-1} text="static" /> : null
        ];

        for (let i = 0; i < collectionOne.length; i++) {
          content.push(<Loop key={i} text={collectionOne[i]} />);
        }
  ...
  describe("VFragment within other nodes", () => {
    it("Should not clear nodes when non keyed", () => {
      const Nodes = ({ items }) => {
        ...
        for (let i = 0; i < items.length; i++) {
          content.push(<span>{items[i]}</span>);
        }

        content.push(<div>end</div>);
    ...
    it("Should not clear nodes when keyed inside vFragment", () => {
      const Nodes = ({ items }) => {
        const content = [<div key="test">test</div>];

        for (let i = 0; i < items.length; i++) {
          content.push(<span key={items[i]}>{items[i]}</span>);
        }

        content.push(<div key="end">end</div>);
  ...
  describe("Forced keyed children", () => {
    it("Should always go keyed algorithm when parent has hasKeyedChildren", () => {
      const Collection = ({ children }) => <div>{children}</div>;
    ...
    it("Should be able to swap from keyed to nonkeyed when nextNode no longer is keyed", () => {
      const CollectionKeyed = ({ children }) => <div>{children}</div>;

      const CollectionNonKeyed = ({ children }) => <div>{children}</div>;
  ...
  describe("Children lifecycle with fastUnmount", () => {
    ...
    it("Should fastUnmount child component when only parent has unmount callback", done => {
      ...
      setTimeout(() => {
        ...
        expect(dirtyReference.__UN).toEqual(true);
